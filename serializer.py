...
from schema import (UserGrapheneInputModel,UserGrapheneModel, PostGrapheneInputModel, PostGrapheneModel, CommentGrapheneInputModel, CommentGrapheneModel)
...
class Query(graphene.ObjectType):
    ...
    @staticmethod
    def resolver_list_users(parent, info):
        return User.all()    
        
    @staticmethod
    def resolver_get_single_user(parent, info, user_id):
...
class CreatePost(graphene.Mutation):
    ...
    def mutate(parent, info, post_details):
        user = User.find_or_fail(port_details.user_id)
        post = Post()
